...
`console.js` switches views from `console-data.js`. Views: Platform Topology, Configuration, Delivery, Infrastructure, Services & Spend, **Headroom Demo**, **LiteLLM Admin UI**. Banner is Demo Mode – Read-Only except live probes and the Cognito-gated Admin link. Headroom POSTs `https://n8n.levkesha.com/webhook/headroom-demo` (n8n → ClusterIP `:8787` `/v1/compress`). LiteLLM primary CTA opens `https://litellm.levkesha.com/ui` (ALB Cognito → ClusterIP `:4000`); secondary health probe POSTs `https://n8n.levkesha.com/webhook/litellm-demo`. Break-glass: `kubectl -n llm-cost port-forward svc/litellm 4000:4000`. No invented ratios/health on failure.
